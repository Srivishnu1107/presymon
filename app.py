...
def ai():
    ...
    if cpu > 85:
        tip = "⚠ CPU very high — close heavy apps"
    elif ram > 85:
        tip = "⚠ RAM almost full — restart recommended"
    elif disk > 90:
        tip = "⚠ Disk nearly full — cleanup required"
    else:
        tip = "✅ System healthy"
...
def ai_chat():
    ...
    if "status" in q or "health" in q:
        return jsonify({
            "reply":
            f"System Status → CPU:{cpu}% | RAM:{ram}% | Disk:{disk}%\n"
            "Overall system is stable and operational ✅"
        })
    ...
    if "cpu" in q:
        return jsonify({
            "reply":
            f"CPU usage is {cpu}%.\n"
            + ("Processor is under heavy load 😅" if cpu > 80 else "CPU performance is normal 👍")
        })
    ...
    if "ram" in q or "memory" in q:
        return jsonify({
            "reply":
            f"RAM usage is {ram}%.\n"
            + ("Memory almost full — close background apps." if ram > 80 else "Memory usage is healthy.")
        })
    ...
    if "disk" in q or "storage" in q:
        return jsonify({
            ...
            + ("Storage space running low ⚠" if disk > 85 else "Storage capacity is sufficient.")
        })
    ...
    if "suggest" in q or "improve" in q or "optimize" in q:
        return jsonify({
            ...
            "• Limit heavy background processes"
        })
    ...
    if "joke" in q:
        return jsonify({
            "reply":
            "Why don’t computers panic?\nBecause they keep their cache 😎"
        })

    if "bored" in q:
        return jsonify({
            "reply":
            "Maybe close some tabs… or touch grass 🌱"
        })
